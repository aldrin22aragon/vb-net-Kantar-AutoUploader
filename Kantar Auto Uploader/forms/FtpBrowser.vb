    ...
    Private Sub Bg_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles Bg.DoWork
        ...
        If Not processing Then
            ...
            Try
                ...
                Dim balikIsangBeses As Boolean = True
balikDito:
                Try
                    Dim directory As WinSCP.RemoteDirectoryInfo = ses.ListDirectory(path)
                    fls = directory.Files
                Catch ex As Exception
                    Dim string_noDirectory As String = "No such file or directory"
                    If ex.Message.ToUpper.Contains(string_noDirectory.ToUpper) AndAlso balikIsangBeses Then
                        MyMessageBox.Info($"Directory ""{path}"" not found. Will redirect to root ""/""")
                        balikIsangBeses = False
                        path = "/"
                        GoTo balikDito
                    End If
